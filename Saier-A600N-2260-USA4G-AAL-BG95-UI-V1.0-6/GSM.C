...
const unsigned	char  overtime[]=	"T";
const unsigned	char  overram[]=	"B";
...
unsigned char gettime(void)
{
  ...
  unsigned char R_YEAR_teep;			
  unsigned char R_MONTH_teep;			
  unsigned char R_DAY_teep;			
  unsigned char R_CLOCK_HOUR_teep;				
  unsigned char R_CLOCK_MINUTE_teep;				
  unsigned char R_CLOCK_SECOND_teep; 
  k=0;	
  _emi=0;
  for(i=0;i<Rx_Size;i++)
    {
	  if(Rx_Buff0[i]=='\"')
	   {
	     ...
	     R_YEAR_teep =time[0]*10+time[1];  
	     R_MONTH_teep =time[3]*10+time[4];			
	     R_DAY_teep =time[6]*10+time[7];			
	     R_CLOCK_HOUR_teep =time[9]*10+time[10];				
	     R_CLOCK_MINUTE_teep =time[12]*10+time[13];				
	     R_CLOCK_SECOND_teep =time[15]*10+time[16]; 	     
         if(R_YEAR_teep<80&&R_YEAR_teep>21&&R_MONTH_teep<13&&R_MONTH_teep>0&&R_DAY_teep<32&&R_CLOCK_HOUR_teep<25&&R_CLOCK_MINUTE_teep<61)                       
		   {
					   												
				 R_YEAR=	R_YEAR_teep;				
				 R_MONTH=	R_MONTH_teep;				
				 R_DAY=		R_DAY_teep;				
				 R_CLOCK_HOUR=		R_CLOCK_HOUR_teep;				
				 R_CLOCK_MINUTE=	R_CLOCK_MINUTE_teep;				
				 R_CLOCK_SECOND=	R_CLOCK_SECOND_teep;
...
//--------------------------------------------------- 		  		    											
		    case 35:
					...
					//	printb((unsigned char*)urlHead,sizeof(urlHead));
						print((unsigned char*)urlHead);
						print((unsigned char*)"\r\n\{\r\n");
						
						print((unsigned char*)"    \"IMEI\":");						
						printb((unsigned char*)up_data.IMEI,sizeof(up_data.IMEI));
						print((unsigned char*)",\r\n");
						

						print((unsigned char*)"    \"SN\":");
						...
						print((unsigned char*)"    \"Check Data\":\r\n    {\r\n");
																

						print((unsigned char*)"        \"Model\":\"A600N\",\r\n");
						

						print((unsigned char*)"        \"ver\":\"A600N_1.0\",\r\n");
							
										

						print((unsigned char*)"        \"User\":\"");						
						DRV_PutChar(up_data.USER_BUF);												
						print((unsigned char*)"\",\r\n");	
	
						print((unsigned char*)"        \"SYS\":\"");					
						printb((unsigned char*)up_data.SYS,sizeof(up_data.SYS));
						print((unsigned char*)"mmHg\",\r\n");

						print((unsigned char*)"        \"DIA\":\"");
...
void GSM_OFF(void)
{
	 print_with_Enter((unsigned char*)"\r\nAT+QIDEACT=1");
	 if(read_GSM_data(10)==UART_BACK_OK)
	 {      
	 }
	 else
	 ...
	 print_with_Enter((unsigned char*)"AT+QPOWD");
	 if(read_GSM_data(10)==UART_BACK_OK)
	 {      
	 }
	 if(read_GSM_data(10)==UART_BACK_DOWN)
	 {      
	 }  
